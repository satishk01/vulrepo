/**
 * Central configuration for claude-secaudit (multi-provider, BYOK).
 *
 * Model aliases now live in src/providers/registry.js and resolve to a
 * { provider, modelId } pair. See that file and README "Providers & models".
 */

import { resolveModel as _resolveModel, MODEL_REGISTRY } from './providers/registry.js';

// Re-exported for any code/tests that referenced the old name.
export const MODEL_ALIASES = MODEL_REGISTRY;

export const DEFAULTS = {
  region: process.env.AWS_REGION || 'us-east-1',
  models: ['bedrock-sonnet-4.5'],
  concurrency: 4,
...
/**
 * Resolve a model name/alias to { alias, provider, id }.
 * (`id` kept as the field name the analyzer/report already use.)
 */
export function resolveModelId(nameOrId, userConfig) {
  const { alias, provider, modelId } = _resolveModel(nameOrId, userConfig);
  return { alias, provider, id: modelId };
}
