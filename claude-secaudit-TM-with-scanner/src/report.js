...
function findingCard(f) {
  ...
  const sourceTag = f.source === 'scanner'
    ? `<span class="tag tag-src tag-scan">scanner</span>`
    : f.source === 'corroborated'
      ? `<span class="tag tag-src tag-corr">AI + scanner</span>`
      : `<span class="tag tag-src tag-ai">AI</span>`;
...
export function renderReport({ findings, stats, meta }) {
  ...
  const modelList = meta.models.map((m) => `${esc(m.alias)} <span class="muted">[${esc(m.provider || 'bedrock')}] (${esc(m.id)})</span>`).join(', ');
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .tag-src{font-weight:700}
  .tag-ai{color:#6c3483;background:#f4ecf7}
  .tag-scan{color:#1e6b52;background:#e8f6f0}
  .tag-corr{color:#7d3c00;background:#fdf2e3}
...
<div class="wrap">
  ...
  <section>
    ...
    <div class="meta-grid">
      ...
      <div class="meta-cell"><div class="k">Mode</div><div class="v">${esc(meta.mode || 'llm')}</div></div>
      <div class="meta-cell"><div class="k">Files analyzed</div><div class="v">${stats.filesAnalyzed ?? '—'}</div></div>
      ${meta.models && meta.models.length ? `<div class="meta-cell"><div class="k">AI models</div><div class="v">${modelList}</div></div>` : ''}
      ${meta.models && meta.models.length ? `<div class="meta-cell"><div class="k">Providers</div><div class="v">${esc((meta.providers || []).join(', '))}${meta.region ? ` <span class="muted">(region ${esc(meta.region)})</span>` : ''}</div></div>` : ''}
      ${meta.scanners && meta.scanners.length ? `<div class="meta-cell"><div class="k">Scanners run</div><div class="v">${meta.scanners.map((s) => `${esc(s.name)}${s.error ? ` <span class="muted">(${esc(s.error)})</span>` : ` <span class="muted">(${s.findings})</span>`}`).join(', ')}</div></div>` : ''}
      ${meta.scannersMissing && meta.scannersMissing.length ? `<div class="meta-cell"><div class="k">Scanners not installed</div><div class="v"><span class="muted">${esc(meta.scannersMissing.join(', '))}</span></div></div>` : ''}
      <div class="meta-cell"><div class="k">Duration</div><div class="v">${esc(meta.duration)}</div></div>
      ${meta.usage && (meta.usage.requests || 0) > 0 ? `<div class="meta-cell"><div class="k">Tokens (in / out)</div><div class="v">${meta.usage.inputTokens.toLocaleString()} / ${meta.usage.outputTokens.toLocaleString()}</div></div>` : ''}
  ...
  <footer>
    <div>Generated by <b>claude-secaudit</b> v${esc(meta.version)} — multi-provider AI security review (Bedrock · Anthropic · Google · OpenRouter · Azure · Ollama).</div>
